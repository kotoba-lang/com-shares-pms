(ns shares-pms.main
  "Kotodama WASM entrypoint for the shares_pms clean-room actor (L4) — Clojure port.
  ...
  facts (`shares_pms.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "shares_pms")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"shares_pms.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "shares_pms-compat" :tier tier :entities entities} 200])
...
(defn start! [] :shares_pms-compat/ready)
